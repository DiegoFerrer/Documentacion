...
<script>
	...
	// ------  CLASES
	  ...
	  //POO con Clases
	    ...
	    //Los métodos no se declaran de forma explícita con var, let o const  y todos deben retornar algo
	...
	class Name {
		constructor(parametro1, parametro2, etc){
			// propiedades
			this.nombre = parametro1
			this.edad = parametro2
			this.algo = parametro3
		}
		// codigo de la clase, generalmente metodos
	}

	// Creando instancias
	let diego = new Name('diego', '23','Argentina')
	// para utilizar esta clase en otro archivo hay que exportarla
	export Name
	// luego en el archivo a utilizar la clase importarla
	import name from 'direccion del archivo que tiene la clase'
  ...
  // --------------------------------------------------------
	...
	/* Para compartir propiedades con otras clases,se recomienda crear una clase con estas propiedades y que otras la hereden : */
	class player {codigo compartido}

	class futbolista extends player {
    constructor(parametrosDelPadre y propios){
      super(parametrosDelPadre) // hace referencia al constructor del padre
      this.parametroPropio1 = parametroPropio
    }
  }

	/* La clase futbolista estan heredando la funcionalidad de la clase player
	basicamente se puede heredar una funcion constructora o funciones propias
	del lenguaje,como date, con la palabra extends
	 */
